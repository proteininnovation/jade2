...
from jade2.rosetta_jade.flag_util import get_common_flags_string_for_init
from jade2.basic.path import get_decoy_name
...
def get_parser():
    ...
    parser.add_argument("--skip_adapt_numbering",
                        help = "Keep the PDBInfo intact after adding residues if possible",
                        default = False,
                        action="store_true")
    return parser
...
if __name__ == "__main__":
    ...
    if (rosetta_start+1 != rosetta_end):
    ...
    #Numbering adaptation
    use_current_numbering = False
    insert_length = len(options.sequence)

    start_num = int(options.start[0:-1])
    end_num = int(options.stop[0:-1])
    chain = str(options.start)[-1]
    if end_num - start_num >=insert_length:
        use_current_numbering = True

    added_residues = 0

    alphabets = list(string.ascii_lowercase)
    alphabets.extend(list(string.ascii_uppercase))
    ...
    for i in range(seq_start, seq_end +1):
        ...
        if not options.skip_adapt_numbering:
            if use_current_numbering:
                pose.pdb_info().set_resinfo(current_resnum+1, chain, start_num+added_residues, "B")
            else:
                pose.pdb_info().set_resinfo(current_resnum+1, chain, seq_start, alphabets[added_residues-1])
