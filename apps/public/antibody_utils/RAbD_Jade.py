...
from jade2.RAbD.AnalyzeAntibodyDesigns import CompareAntibodyDesignStrategies

## Window Frames
from jade2.RAbD.window_main.menu import AntibodyDesignAnalysisMenu
from jade2.RAbD.window_main.CompareStrategiesFrame import CompareStrategiesFrame
from jade2.RAbD.window_main.FeaturesFrame import FeaturesFrame
from jade2.RAbD.window_main.AnalysisFrame import AnalysisFrame

## Window Modules
from jade2.RAbD.window_modules.FilterSettingsWindow import *

from jade2.basic.TKinter.ImageFrame import ImageFrame
...
class CompareAntibodyDesignStrategies_GUI:
    ...
    def exit(self):
        if threads.n_alive() > 1:
            if tkinter.messagebox.askyesno(title = "Exit?", message = repr(threads.n_alive())+" Processes still running.  Exit?"):
                threads.kill_all()
                self._tk_.destroy()
            else:
                self._tk_.mainloop()
