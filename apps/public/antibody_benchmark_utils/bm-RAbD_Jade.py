...
import jade2.RAbD_BM.tools as tools
from jade2.rosetta_jade.BenchmarkInfo import *
...
class AnalyzeBenchmarksGUI:
    ...
    def select_bm_list_reload(self):
        infilename = tkinter.filedialog.askopenfilename(initialdir=self.current_dir, title="Select Benchmark List file")
    ...
    def set_db(self, title, variable):
        tkinter.simpledialog.askstring(title=title, variable=variable, prompt = "Set DB")
...
class CompareBenchmarks_GUI:
    ...
    def add_main_benchmark(self):
        benchmark_name = tkinter.simpledialog.askstring(title = "benchmark", prompt = "benchmark Name")
    ...
    def read_from_main_set_benchmarks(self):
        if not self.compare_benchmarks.main_dir.get():
            ...
            strat_dir = tkinter.filedialog.askdirectory(initialdir = self.current_dir, title = "benchmark Analysis Directory")
            if not strat_dir:
                return
    ...
    def set_reference_db(self):
        d = tkinter.filedialog.askopenfilename(title = "Reference DB", initialdir = self.current_dir)
        if not d: return
        self.compare_benchmarks.reference_db.set(d)

    def set_scorefunction(self):
        score = tkinter.simpledialog.askstring(title="Score", prompt = "Set Scorefunction", initialvalue = self.compare_benchmarks.scorefunction.get())
        if not score:
            return
        self.compare_benchmarks.scorefunction.set(score)

    def split_recovery_db(self):
        db = tkinter.filedialog.askopenfilename(initialdir = self.current_dir, title = "Combined recovery database")
        if not db: return
    ...
    def setup_custom_cdr_data_plot(self):
        type = tkinter.simpledialog.askstring(title = "Data Type", prompt = "Please enter a custom CDR data to plot against")
        if not type: return
...
class CDRData:
    ...
    def _add_data(self, benchmark, decoy_data_map):
        ...
        """
        if benchmark not in self.all_data:
