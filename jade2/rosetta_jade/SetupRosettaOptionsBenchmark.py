import os
import sys
from jade2.rosetta_jade.SetupRosettaOptionsGeneral import SetupRosettaOptionsGeneral
...
class SetupRosettaOptionsBenchmark(SetupRosettaOptionsGeneral):
    ...
    def get_benchmark_names(self, only_rosetta = False):
        ...
        for key in self.json_dict:
            if type(self.json_dict[key]) == dict and self.key_benchmarks in self.json_dict[key]:
                if only_rosetta and type(self.json_dict[key]) == dict and self.key_rosetta_option in self.json_dict[key]:
    ...
    def get_non_rosetta_option_benchmark_names(self):
        ...
        for key in self.json_dict:
            if type(self.json_dict[key]) == dict and self.key_benchmarks in self.json_dict[key]:
                if  self.key_rosetta_option not in self.json_dict[key]:
                    benchmark_names.append(key)
                else:
                    continue
    ...
    def get_benchmarks_of_key(self, benchmark_name):
        ...
        :rtype: list
        """
        if benchmark_name not in self.json_dict:
    ...
    def use_benchmark_for_outdir(self, benchmark):
        ...
        if benchmark in self.json_dict:
            if self.key_use_for_outdir in self.json_dict[benchmark]:
    ...
    def use_benchmark_for_prefix(self, benchmark):
        ...
        if benchmark in self.json_dict:
            if self.key_use_for_prefix in self.json_dict[benchmark]:
