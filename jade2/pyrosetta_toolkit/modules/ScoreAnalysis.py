...
#Toolkit Imports
from jade2.pyrosetta_toolkit.modules.tools.analysis import rmsd
...
class ScoreAnalysis:
    ...
    def get_top_scoring_by_percent(self, percent=False, ask_copy_results=True):
        ...
        if not percent:
            ...
            percent = tkinter.simpledialog.askfloat(title="Cutoff", prompt="Please enter a percent for top cutoff", initialvalue=.05)
    ...
    def get_top_scoring_by_number(self, top_number=False, ask_copy_results = True):
        ...
        if not self.score_pairs:print("Please load scores.");return
        if not top_number:
            top_number = tkinter.simpledialog.askinteger(title="Cutoff", prompt="Please enter how many top scoring poses you would like", initialvalue=10)
    ...
    def get_score_vs_rmsd(self, native_pose, loops_as_strings):
        ...
        for pair in self.score_pairs:
            ...
            if loops_as_strings:
                for loop_string in loop_rms_map:
                    if loop_string not in self.score_rmsd_triplet_loops:
        ...
        #Output Results
        outpath = tkinter.filedialog.asksaveasfilename(title="Save as..", initialdir=global_variables.current_directory)
        if not outpath:
            return
    ...
    def copy_results(self, path_dict, type, ask = True, dir_suffix = ""):
        ...
        """
        if ask:
            result = tkinter.messagebox.askyesno(title="Copy", message="Copy resultant structures?")
