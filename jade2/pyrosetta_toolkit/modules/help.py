...
def about():
    ...
    tkinter.messagebox.showinfo(title="About", message=message)
...
*To specify a single residue, use the same PDB number for start and end."
    tkinter.messagebox.showinfo(title="Region Selection", message=message)

def mutSA():
    message = "Accessible Surface Area: Square Angstroms, (Chothia 1976)"
    tkinter.messagebox.showinfo(title="Design Help", message=message)
def mutRM():
...
(c) For more information, see http://www.rosettacommons.org. Questions about this can be \n \
(c) addressed to University of Washington CoMotion, email: license@uw.edu."
    tkinter.messagebox.showinfo(title="License", message=message)
...
*comparative model = a protein model where the primary sequence from one protein (target) is placed, or threaded, onto the three dimensional coordinates of a protein of known structure (template)language (binary)"
    tkinter.messagebox.showinfo(title="Rosetta Glossary from Nature Protocols Paper. Written by Stephanie Hirst DeLuca", message=message)
...
def scwrl():
    ...
    tkinter.messagebox.showinfo(title="SCWRL Info", message=message)
