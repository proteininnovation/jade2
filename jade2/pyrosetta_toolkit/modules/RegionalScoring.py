...
class RegionalScoring():
    ...
    def ret_n_hbonds(self):
        ...
        core.scoring.hbonds.fill_hbond_set(self.pose, False, set)
        return set.nhbonds()
    ...
    def ret_energy_string(self):
        ...
        for type in self.scoretypes:
            if ( self.weights[ type ] != 0.0 ):
                out=out+ ' ' +  core.scoring.name_from_score_type(type).ljust(24,) +(" %.3f"%self.weights[ type ]).ljust(9)+"   "
...
#### Switches Don't use these - deprecated####
    ...
    def updateweights(self):
        ...
        self.scoretypes = []
        for i in range(1, int(core.scoring.end_of_score_type_enumeration)+1):
            ii = core.scoring.ScoreType(i)
