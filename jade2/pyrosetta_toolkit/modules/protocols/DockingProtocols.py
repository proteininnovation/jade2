...
#Rosetta Imports
from rosetta import *
from rosetta.protocols.task_operations import *
...
class DockingProtocols(ProtocolBaseClass):
    ...
    def low_res_dock(self):
        ...
        if self.score_class.ScoreType.get()!= "interchain_cen":
            result = tkinter.messagebox.askyesnocancel(title="Set Docking scorefunction?", message="Docking requires the appropriate scorefunction (interchain_cen).  Temporarily set docking scorefunction?")
            if result:
        ...
        to_dock = tkinter.simpledialog.askstring(title = "Input", prompt="Please supply two partners (ex - A_B or A_BC ).  The first will be held rigid.")
        if not to_dock: return
        to_dock = to_dock.upper()

        repack_interface = tkinter.messagebox.askyesno(message="Repack interface post-dock (talaris2013)?")
    ...
    def high_res_dock(self):
        ...
        if self.score_class.ScoreType.get()!="docking":
            result = tkinter.messagebox.askyesnocancel(title="Set docking scorefunction?", message="Standard docking scorefunction not set. Temporarily set scorefunction?")
            if result:
                start_scorefxn = ScoreFunction()
                start_scorefxn.assign(self.score_class.score)
