...
class LowResLoopModelingProtocols(ProtocolBaseClass):
     ...
     def default_CCD(self, fragset=False, fraglength=False):
          ...
          if self.score_class.ScoreType.get()!="cen_std" != self.score_class.ScorePatch.get() != "score4L":
               result = tkinter.messagebox.askyesnocancel(title="Set loop modeling scorefunction?", message="Standard loop modeling scorefunction not set (cen_std/score4L patch). Temporarily set scorefunction?")
               if result:
          ...
          if not fragset:
               fragset = tkinter.filedialog.askopenfilename(initialdir = global_variables.current_directory, title="Fragset File")
               if not fragset:return

          if not fraglength:
               fraglength = tkinter.simpledialog.askinteger(title="Fragment Length", initialvalue=3)

          extend = tkinter.messagebox.askyesno(title="Extended", message="Start with idealized bond lenghths, angles, and discard original phi/psi?")
          movemap=MoveMap()
