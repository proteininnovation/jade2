...
#Toolkit Imports
from jade2.pyrosetta_toolkit.modules.Region import *
...
class FloppyTailProtocol(ProtocolBaseClass):
    ...
    def setup_protocol(self, main):
        ...
        if not self.input_class.regions.get_num_regions():
            ...
            return
        result = tkinter.messagebox.askokcancel(title="Continue?", message="Production runs require ~30,000 decoys at ~ 1 decoy/hr/processor. Continue?")
        ...
        try:
            self.sep = tkinter.ttk.Separator(self.main, orient=HORIZONTAL)
        except NameError:
            pass
    ...
    def get_and_set_resfile(self):
        self.resfile.set(tkinter.filedialog.askopenfilename(title="Resfile", initialdir = global_variables.current_directory))
    ...
    def get_and_set_fragfile(self):
        self.frag3.set(tkinter.filedialog.askopenfilename(title="Frag3", initialdir = global_variables.current_directory))
