...
class AnalysisProtocols(ProtocolBaseClass):
    ...
    def analyze_vip(self):
        ...
        tkinter.messagebox.showinfo(title = 'note', message = message)
        if not (tkinter.messagebox.askyesno(message="Continue?")): return
        cycles = tkinter.simpledialog.askinteger(title = "Cycles", prompt="Please enter max cycles (0 indicates that the protocol will run until favorable mutations are found)", initialvalue=get_integer_option('cp:ncycles'))
