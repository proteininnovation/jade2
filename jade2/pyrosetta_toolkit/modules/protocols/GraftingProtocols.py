...
class GraftMoverWindow(ProtocolBaseClass):
    ...
    def load_from_pose(self):
        filename = tkinter.filedialog.askopenfilename(title="PDB with insert OR insert PDB", initialdir=global_variables.current_directory)
    ...
    def run_graft(self):
        ...
        if re.search("Double Loop", self.graft_type.get()):
            nter_overhang = tkinter.simpledialog.askinteger(title='N-terminal Overhang', prompt='Please enter the number of residues N-terminal from insert_start to use for superposition', initialvalue=3)
            if not nter_overhang:return
            cter_overhang = tkinter.simpledialog.askinteger(title='C-terminal Overhang', prompt='Please enter the number of residues C-terminal from insert_end to use for superposition', initialvalue=3)
