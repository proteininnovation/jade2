...
from . import loops
#from jade2.pyrosetta_toolkit.modules.SQLPDB import *
from jade2.pyrosetta_toolkit.modules.tools import loops as loop_tools
from jade2.pyrosetta_toolkit.window_main import global_variables
from jade2.pyrosetta_toolkit.modules.definitions import restype_definitions
...
def save_loop_file(p, regions, ask_info=True, outfilename=False):
    ...
    if ask_info:
        ...
        ask_cut_points = tkinter.messagebox.askyesno(message="Define Cutpoints?", default=tkinter.messagebox.NO)

        discard_loops =  tkinter.messagebox.askyesno(message = "Discard Phi/Psi and build using ideal bond lengths and angles?")
    else:
        ask_cut_points = False
        discard_loops = True
    if not outfilename:
        outfilename = tkinter.filedialog.asksaveasfilename(initialdir = global_variables.current_directory, title="Output loop file to...")
    ...
    for region in regions:
        ...
        if ask_cut_points:
            ...
            while not cutpoint_known:
                cut = tkinter.simpledialog.askstring(title="cutpoint", prompt="Cutpoint for Loop (#, default, 0 for random) " +loop_string, initialvalue="default")
                if cut== "default":
                    cut = (end - start)/2
                    cut = start+cut
                    cutpoint_known=True
                    ...
                    if ((int(cut) < start) | (int(cut) > end)):
                        tkinter.messagebox.showerror(message="Invalid CutPoint!")
                        cut = int(cut)
                        cutpoint_known=False
        else:
            cut = (end - start)/2
...
def clean_whitespace(obj):
    if isinstance(obj, str):
        ...
        return dict((k, clean_whitespace(v)) for (k,v) in list(obj.items()))
    else:
        return obj
...
def save_basic_resfile(p, outname=None):
    ...
    ResDic = dict()
    if not outname:
        outname = tkinter.filedialog.asksaveasfilename(initialdir = global_variables.current_directory, title="Output resfile to...")
...
def saveSeqFile(p, fileout=None, loops_as_strings=None):
    ...
    if not p.total_residue():
        ...
        return
    if not fileout:
        fileout = tkinter.filedialog.asksaveasfilename(initialdir=global_variables.current_directory)
...
def save_basic_blueprint(p, output=True, outfilename=None):
    ...
    if output:
        if not outfilename:
            outfilename = tkinter.filedialog.asksaveasfilename(initialdir = global_variables.current_directory)
            if not outfilename:return
        global_variables.current_directory=os.path.dirname(outfilename)
...
def make_PDBLIST(directory=""):
    ...
    """
    if directory=="":
        directory = tkinter.filedialog.askdirectory(title = "Choose directory with PDB files", initialdir = global_variables.current_directory)
        if not directory: return
        global_variables.current_directory=directory

    contains = tkinter.simpledialog.askstring(title="Contains...", prompt="Separate mutliple match criteria by a coma...", initialvalue=".pdb")
...
def make_PDBLIST_recursively(directory=""):
    if directory=="":
        directory = tkinter.filedialog.askdirectory(initialdir = global_variables.current_directory)
        if not directory: return
        global_variables.current_directory=directory
...
def convert_PDBLIST_to_sqlite3db(pdblist_path):
    ...
    if not pdblist_path:
        ...
        return
    structID = tkinter.simpledialog.askstring(title="structID", prompt="These entries will have a structID of...", initialvalue="na")
...
def extract_pdb_from_sqlite3db():
    dbfilename = tkinter.filedialog.askopenfilename(initialdir = global_variables.current_directory, title = "Database filename")
    if not dbfilename: return
    global_variables.current_directory = os.path.dirname(dbfilename)

    pdbID = tkinter.simpledialog.askstring(title = "pdbID", prompt="Please enter the pdbID/filepath you wish to extract")
    if not pdbID: return
    ...
    outname = tkinter.filedialog.asksaveasfilename(initialdir = global_variables.current_directory)
    if not outname: return
    global_variables.current_directory = os.path.dirname(outname)
...
def extract_pdbs_from_sqlite3db(pdblist_path):
    ...
    pdbdb = PDB_database(dbfilename)
    print("Database Opened")
    outdir = tkinter.filedialog.askdirectory(initialdir = global_variables.current_directory, title= "Choose output directory")
    global_variables.current_directory = outdir

    keep_original_filename = tkinter.messagebox.askyesno(title="Keep Original Filename?", message="Add numerical designation to filename to keep from overwriting same PDBs in list?", default=tkinter.messagebox.NO)
    strucID = tkinter.simpledialog.askstring(title="strucID", prompt="Extract entries with structID of...", initialvalue="na")
    PDBLIST = open(pdblist_path, 'r')
...
def score_PDBLIST(pdblist_path, score, processors, output_class = None):
    ...
    if not pdblist_path:
        pdblist_path = tkinter.filedialog.askopenfilename(initialdir = global_variables.current_directory, title = "PDBLIST")
    ...
    #Multiprocessing - Slower for extremely small PDBs, great for large ones.
        ...
        for path in list(result_map.keys()):
...
def save_FASTA(pose, base_name, outfilename = None, regions = None ):
    ...
    if not outfilename:
        outfilename = tkinter.filedialog.asksaveasfilename(initialdir = global_variables.current_directory, title="Output FASTA to...")
        if not outfilename: return
        global_variables.current_directory = os.path.dirname(outfilename)
...
def exportPDBSCORE(score):
    ...
    PDBLIST = tkinter.filedialog.askopenfile(title = "PDBLIST", initialdir = global_variables.current_directory)
    OUTFILE = tkinter.filedialog.asksaveasfile(title = "Save As...", initialdir = global_variables.current_directory)
...
def output_molfile_to_params():
    ...
    mdl_file = tkinter.filedialog.askopenfilename(initialdir = global_variables.current_directory, title = "Open MDL, MOL, MOL2, or SDF file")
    if not mdl_file:return
    global_variables.current_directory=os.path.dirname(mdl_file)

    output_kinemage = tkinter.messagebox.askyesno(title = "kinemage", message="Output kinemage file for ligand visualization?")
...
def save_param_path_list(array_of_paths, outfilename=False):
    ...
    if not array_of_paths:
        ...
        return
    if not outfilename:
        outfilename = tkinter.filedialog.asksaveasfilename(initialdir = global_variables.current_directory, title="Output Parm pathList to...")
    if not outfilename:return
    global_variables.current_directory = os.path.dirname(outfilename)
    FILE = open(outfilename, 'w')
    d = dict()
