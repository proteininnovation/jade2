...
def tk_get_directory():
    ...
    dir_name = tkinter.filedialog.askdirectory(initialdir=global_variables.current_directory, title='Pick a directory')
...
def tk_get_file():
    ...
    """
    filename = tkinter.filedialog.askopenfilename(initialdir=global_variables.current_directory, title='Pick a file')
    if not filename:
        return
...
def add_constraints_to_pose_and_scorefunction(pose, score, default_weight = 1.0, constraint_types = False, constraint_file=False):
    ...
    if not constraint_file:
        constraint_file = tkinter.filedialog.askopenfilename(initialdir=global_variables.current_directory, title = "Open Constraint File")
