...
import jade2.pyrosetta_toolkit.modules.tools.loops as loop_tools
    ...
    for line in File:
        ...
        if lineSplit[0]!="pdb":
            ...
            for i in range(3, (len(lineSplit)/2)+2):
                if lineSplit[1] not in fascData:
...
def analyze_interface(p, scorefxn, toolkit = None):
    ...
    if (p.conformation().num_chains()==2):
        ...
        chains = tkinter.simpledialog.askstring(title = "-fixedchains", prompt = "Please input chains to keep fixed  - seperated by a space")
        chains.upper()
        if (chains):
            chains = chains.split()
        else:return

    pack_together = tkinter.messagebox.askyesno(message="rePack before separation")
    pack_separated = tkinter.messagebox.askyesno(message="rePack after separation (Recommended)")
...
def analyze_vip(p, scorefxn):
    ...
    time.sleep(6)
    if (tkinter.messagebox.askyesno(message="Continue?")):
        pass
    else:
        return
    cycles = tkinter.simpledialog.askinteger(title = "Cycles", prompt="Please enter max cycles", initialvalue=rosetta.basic.options.get_integer_option('cp:ncycles'))
