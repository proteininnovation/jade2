...
class ScoreFxnControl():
    ...
    def __init__(self):
        self.weight_dir = basic.database.full_name("scoring/weights")
        self.wd = self.location()[0]
        self.readDefaults()
        self.score = core.scoring.get_score_function()
        ...
        self.pose = core.pose.Pose(); #Should this be here?
    ...
    def setDefaults(self):
        ...
        if "PATCH" in self.defaultDic:
            self.ScorePatch.set(self.defaultDic["PATCH"])
        if "WD" in self.defaultDic:
    ...
    def changeWeight(self, box):
        ...
        termSP = term.split(";")
        if len(termSP)>=2:
            newWeight = tkinter.simpledialog.askfloat(title="New", prompt = "Enter New Weight", initialvalue=float(termSP[1]))
            ...
            newWeight = tkinter.simpledialog.askfloat(title="weight", prompt = "Enter New Weight")
    ...
    def saveAS(self, outfilepath=False):
        ...
        if not outfilepath:
            outfilepath= tkinter.filedialog.asksaveasfilename(initialdir = global_variables.current_directory)
        if not outfilepath:
            return
        self.zero, self.nonzero = self.scoreOption("Breakdown ScoreFxn")
    ...
    def saveToWD(self):
        name = tkinter.simpledialog.askstring(title = "Save", prompt = "Name")
        if not name:
            return
    ...
    def setWD(self):
        self.wd = tkinter.filedialog.askdirectory(title = "Set WD", initialdir = self.wd)
