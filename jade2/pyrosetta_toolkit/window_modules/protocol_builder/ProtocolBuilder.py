...
def getProtocolLists(LisBox):
    ...
    rounds = tkinter.simpledialog.askinteger(title = 'rounds', prompt = 'Please enter the number of rounds desired: ', initialvalue = 1)
    ...
    if VarItemSplit[1] == "Vaccinity":
        ...
        length = tkinter.simpledialog.askfloat(title = "vaccinity", prompt = "entry_er Vaccinity Cuttoff (A)", initialvalue = 5.0)
        DicProtocol[p]['neighborcutoff'] = length
        if VarItemSplit[0]=="Pack" and VarItemSplit[1] == "Vaccinity":
            onlyaround = tkinter.messagebox.askquestion(title = "onlyaround", message = "Pack/Relax Given residues in addition to vaccinity?", default = tkinter.messagebox.YES)
            if onlyaround == "yes":
    ...
    if VarItem == "Relax Vaccinity" or VarItem == "Minimize Vaccinity":
        ...
        if vaccinity == [1, 1] and target == [1, 1]:
            tkinter.messagebox.showerror(message = "Everything is fixed!  Remove Protocol and Try Again!")
            return
    ...
    if VarItemSplit[1] == "Interface":
        ...
        length = tkinter.simpledialog.askfloat(title = "interface", prompt = "entry_er Interface Cuttoff (A)", initialvalue = 6.0)
        interface = tkinter.simpledialog.askstring(title = "interface", prompt = "entry_er Interface (A:CD / AB:CD:F)")
        #Options if you are specifically looking for interfaces between multiple chains.
            #Handles Fully Restricking some chains
        intopt = tkinter.messagebox.askquestion(title = "interface", message = "Fix any Interface Chains?", default = tkinter.messagebox.NO)
        if intopt =="yes":
            interfaceKeep = tkinter.simpledialog.askstring(title = "interface", prompt = "entry_er chains to keep fixed (A-B-C) :", initialvalue = "0")
        ...
        if VarItem == "Relax Interface":
            intopt2 = tkinter.messagebox.askquestion(title = 'interface', message = "Restrict any Interface Chains to rePacking?", default = tkinter.messagebox.NO)
            if intopt2 =="yes":
                interfacePack = tkinter.simpledialog.askstring(title="interface", prompt = "Enter chains to only allow repacking (A:B:C)")
    ...
    if VarItemSplit[1] =="Minimize" or VarItemSplit[0] == "Minimize":
        tol = tkinter.simpledialog.askfloat(title = "tolerance", prompt = "Please Enter Tolerance", initialvalue = .1)
...
def getFragLists(LisBox):
    ...
    length = tkinter.simpledialog.askinteger(title="frag", prompt="entry_er Fragment Length", initialvalue=3)
...
class MainProtocols():
    ...
    def kickProtocol(self):
        ...
        if p.total_residue() == 0:
            tkinter.messagebox.showerror(message = "No Pose Loaded!  ")
            return
        decoys.set(tkinter.simpledialog.askstring(title = "decoys", prompt = "Please entry_er the Number of Decoys Desired:", initialvalue = decoys.get()))
        if int(decoys.get()) > 1:
            print(dirnameout.get())
            if dirnameout.get() == "0":
                dirnameout.set(tkinter.filedialog.askdirectory(initialdir = pwd, title = "Pick directory for decoys: "))
                Outname.set(tkinter.simpledialog.askstring(title = "decoy name", prompt = "Please enter a base name for the decoys: ", initialvalue = output.entry_Outname.get()))
        ...
        if repeat >=1:
            ...
            numbers = tkinter.simpledialog.askstring(title = "Simple Repeats", prompt = "Please Specify which protocols to repeat (1-4)", initialvalue = start)
