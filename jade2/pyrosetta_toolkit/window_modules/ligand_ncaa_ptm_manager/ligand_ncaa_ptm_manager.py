...
from jade2.pyrosetta_toolkit.modules.tools import general_tools as gen_tools
from jade2.pyrosetta_toolkit.window_modules.scorefunction.ScoreFxnControl import ScoreFxnControl
...
class ligand_ncaa_ptm_manager:
    ...
    def __init__(self, input_class, score_class, pose):
        ...
        self.fa_std_dir = 'chemical/residue_type_sets/fa_standard'
        self.patch_directory = self.fa_std_dir+ '/patches'
        self.patches_on = basic.database.full_name(self.fa_std_dir+"/patches.txt")

        self.params_directory = self.fa_std_dir+'/residue_types'
        self.params_on = basic.database.full_name(self.fa_std_dir+"/residue_types.txt")
    ...
    def setTk(self, main):
        ...
        self.label_variant = Label(self.main, text="Variant", font=tkinter.font.Font(weight='bold'))
        self.label_three_letter=Label(self.main, text="Code Read", font=tkinter.font.Font(weight='bold'))
        self.label_default = Label(self.main, text="On by Default", font=tkinter.font.Font(weight='bold'))
        ...
        self.e_function_label = Label(self.main, text = "Energy Function Optimization", font=tkinter.font.Font(weight='bold', size=13))
...
### 'Public' Getter and Setters ###
    ...
    def mutate(self, resnum=False):
        if self.current_main_selection.get()=="patch":
            ...
            if not resnum:
                residue = tkinter.simpledialog.askstring(title = "residue", prompt="Enter the residue you wish to mutate (resNum chain)")
    ...
    def __read_patches__(self):
        ...
        self.patch_types = [ x[1] for x in os.walk(self.patch_directory)]
    ...
    def __read_params__(self):
        ...
        """
        self.param_types = [ x[1] for x in os.walk(self.patch_directory)]
    ...
    def __populate_type_map__(self, dictionary, property_class):
        ...
        if property_class.molecule_type.get().lower() in dictionary:
    ...
    def __populate_name_map__(self, dictionary, property_class):
        ...
        """
        if property_class.name.get() in dictionary:
            pass
        else:
    ...
    def __populate_main_map__(self, dictionary, property_class):
        ...
        if property_class.main_type.get().lower() in dictionary:
...
class Properties:
    ...
    def read_properties(self):
        ...
        for line in FILE:
            ...
            lineSP = line.split()
            try:
                if lineSP[0] in self.properties:
