...
from jade2.pyrosetta_toolkit.modules.RegionalScoring import RegionalScoring
from jade2.pyrosetta_toolkit.modules.Region import Region
from jade2.pyrosetta_toolkit.modules.Region import Regions
...
from jade2.pyrosetta_toolkit.window_modules.options_system.OptionSystemManager import OptionSystemManager
#from jade2.pyrosetta_toolkit import main_window
...
######### Functions that cannot be put in input_tools or do not belong in the main frame, as they set a variable within this class. ################
    ...
    def fetch_pdb(self):
        ...
        FILE = urllib.request.urlopen(self.pdb_url+'/'+pdbID.lower()+'.pdb')
    ...
    def select_pose_then_launch_fixpdb(self):
        ...
        infilename = tkinter.filedialog.askopenfilename(initialdir=global_variables.current_directory, title="Select PDB file")
        if not infilename:return
    ...
    def load_pose(self, path):
        ...
        if self.nonstandard_ResidueTypeSet:
            self.toolkit.pose.assign(core.import_pose.pose_from_file(self.nonstandard_ResidueTypeSet, path))
        else:
            core.import_pose.pose_from_file(self.toolkit.pose, self.pdb_path.get())
    ...
    def return_loaded_pose(self, path):
        ...
        p = Pose()
        if self.nonstandard_ResidueTypeSet:
            p.assign(core.import_pose.pose_from_file(self.nonstandard_ResidueTypeSet, path))
        else:
            core.import_pose.pose_from_file(p, self.pdb_path.get())
        return p

    def set_PDBLIST(self):
        infilename = tkinter.filedialog.askopenfilename(initialdir=global_variables.current_directory,title='Open PDBLIST')
        if not infilename:return
        global_variables.current_directory =os.path.dirname(infilename)
        print("PDBLIST set")
    ...
    def load_param_list(self, infilename=False):
        ...
        """
        if not infilename:
            infilename = tkinter.filedialog.askopenfilename(initialdir=global_variables.current_directory,title='Open param pathList file')
        if not infilename: return
    ...
    def load_loop(self, infilename=False):
        ...
        if not infilename:
            infilename = tkinter.filedialog.askopenfilename(initialdir=global_variables.current_directory,title='Open Loop file')
        if not infilename: return
        global_variables.current_directory =os.path.dirname(infilename)
        FILE = open(infilename, 'r')
        loops_as_strings = []
