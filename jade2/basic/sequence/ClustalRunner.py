...
class ClustalRunner:
    ...
    """
    def __init__(self, fasta_path: Union[Path, str], clustal_name: str="clustal_omega", clustal_dir: Union[Path, str, None] = None):
    ...
    def set_fasta_path(self, fasta_path: Union[Path, str]):
        """
    ...
    def set_hard_wrap(self, hard_wrap: int):
        """
    ...
    def set_extra_options(self, extra_options: str = ""):
        """
    ...
    def output_alignment(self, out_dir: Union[Path, str], out_name: str):
        """
        Configure command line and Run Clustal Omega
        """
        if not os.path.exists(str(self.fasta_path)):
            sys.exit("Fasta Path not good!  Cannot continue!")
        if not os.path.exists(str(out_dir)): os.mkdir(str(out_dir))

        print("Running alignment on: "+str(self.fasta_path))
        print("Outputting to: "+str(out_dir))
        print("As - "+out_name)

        if self.clustal_dir:
            clustal = str(self.clustal_dir)+"/"+self.clustal_name
        else:
