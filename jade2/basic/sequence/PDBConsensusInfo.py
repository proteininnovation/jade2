...
from jade2.basic.structure.Structure import ResidueRecord
from jade2.basic.structure.Structure import ResidueRegion
from jade2.basic.structure.Structure import PDBInfo
...
class PDBConsensusInfo():
    ...
    def get_frequency_for_position(self, position, aa):
        if position not in self.freq:
            return 0

        if aa not in self.freq[position]:
            return 0
    ...
    def get_probability_for_position(self, position, aa):
        if position not in self.stats:
            return 0
        if aa not in self.stats[position]:
            return 0
    ...
    def compute_stats(self):
        ...
        for position in sorted(self.freq.keys()):
            ...
            for aa in list(self.freq[position].keys()):
    ...
    def init_data_map(self):
        ...
        for pdb_info in self.pdb_info_list:
            ...
            for residue in pdb_info.get_all_residue_records():
                ...
                if resnum_triple in self.stats:
                    continue
