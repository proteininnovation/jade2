...
def get_cluster_path():
    """
    Get the path to cluster center information.
    :return:
    """
    return get_database_path()+"/antibody"
...
def get_decoy_extension(decoy):
    """
    Return the extension of the decoy.  .pdb, .pdb.gz, .cif, .cif.gz,
    :param decoy: str
    :rtype: str
    """
    #if not decoy: return None
    ...
    for ext in extensions:
        for comp in compressions:
            ...
            if str(decoy).endswith(extension):
...
def get_scorefiles_recursively(inpath, ext=".sc"):
    """
    Get a list of scorefile paths recursively, matching the extension.
    :param inpath:
    :param ext:
    :return:
    """
    scorefiles=[]
    dirs = get_directories_recursively(inpath)
    dirs.append(inpath)
    for d in dirs:
        #print(d)
        files = glob.glob(d + "/" + '*' + ext)
        #print(files)
        scorefiles.extend(files)
    return scorefiles
