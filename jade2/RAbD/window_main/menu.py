...
from jade2.RAbD.AnalyzeAntibodyDesigns import *
...
class AntibodyDesignAnalysisMenu(object):
    ...
    def change_root(self):
        root = tkinter.filedialog.askdirectory(initialdir=self.current_dir, title="Root Directory")
        if not root:
            return
    ...
    def read_from_db_dir_set_strategies(self):
        ...
        if not self.compare_designs.db_dir.get() or not os.path.exists(self.compare_designs.db_dir.get()):
            ...
            db_dir = tkinter.filedialog.askdirectory(initialdir=self.current_dir, title="Database DIR")
            if not db_dir:
                return
    ...
    def set_reference_db(self):
        d = tkinter.filedialog.askopenfilename(title="Reference DB", initialdir=self.compare_designs.db_dir.get())
        if not d: return
        self.current_dir = os.path.dirname(d)
        self.compare_designs.reference_db.set(d)

    def set_top_n(self):
        top = tkinter.simpledialog.askinteger(title="Top N", prompt="Number of top scoring",
                                        initialvalue=self.compare_designs.top_n.get())
        if not top: return
        self.compare_designs.top_n.set(top)

    def set_top_n_combined(self):
        top = tkinter.simpledialog.askinteger(title="Top N Combined", prompt="Number of top scoring Combined",
    ...
    def set_max_clustal_procs(self):
        max = tkinter.simpledialog.askinteger(title="Max P", prompt="Max NP.  Clustal by default uses all.",
                                        initialvalue=self.main_gui.clustal_procs.get())
        if not max: return
        self.main_gui.clustal_procs.set(max)

    def set_clustal_output_format(self):
        f = tkinter.simpledialog.askstring(title="Clustal output format", initialvalue=self.main_gui.clustal_output_format.get())
        if not f:
            return
    ...
    def set_clustal_soft_wrap(self):
        wrap = tkinter.simpledialog.askinteger(title="Wrap", prompt="Set Soft Wrap",
    ...
    def set_native_path(self):
        native_path = tkinter.filedialog.askopenfilename(title="Native path", initialdir=self.current_dir)
        if not native_path:
            return
        else:
    ...
    def set_pyigclassify_dir(self):
        origin_path = tkinter.filedialog.askopenfilename(title = "PyIgClassify Directory", inititaldir = self.current_dir)
        if not origin_path:
            return
        else:
    ...
    def run_clustal_on_all_combined(self):
        ...
        extra_options = tkinter.simpledialog.askstring(title="Extra Options", prompt="Clustal Extra Options",
    ...
    def create_subset_databases(self, score_name):
        rosetta_extension = tkinter.simpledialog.askstring(title="Rosetta Extension",
        ...
        prefix = tkinter.simpledialog.askstring(title="Prefix",
