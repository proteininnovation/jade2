...
from jade2.basic.TKinter.Listbox import AutoListbox
...
class CompareStrategiesFrame( Frame ):
    ...
    def add_main_strategy(self):
        strategy_name = tkinter.simpledialog.askstring(title="Strategy", prompt="Strategy Name")
        if not strategy_name:
            return

        strategy_path = tkinter.filedialog.askdirectory(initialdir=self.main_gui.current_dir, title="Strategy Path")
