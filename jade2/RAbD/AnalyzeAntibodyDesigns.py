...
from jade2.antibody.cdr_data.CDRDataTypes import *
from jade2.antibody.decoy_data.DecoyDataTypes import *
from jade2.basic import fasta
from jade2.basic.filters.DataFilters import *
from jade2.basic.filters.FilterSettings import *
from jade2.basic.threading.Threader import *
from jade2.basic.dataframe import util as PandasDataFrame
from jade2.RAbD_BM.AnalysisInfo import *

# Rosetta Tools
import jade2.rosetta_jade.FeaturesJsonCreator as json_creator
...
class CompareAntibodyDesignStrategies:
    ...
    def get_pandas_dataframe(self):
        """
        Gets a dataframe Dataframe for all
    ...
    def get_top_from_dataframe(self, score_name):
        """
        Gets a dataframe Dataframe for top
        :rtype: pandas.DataFrame
        """
    ...
    def get_top_dataframe_by_all_scores(self):
        """
        Get a dataframe DataFrame for top, grouped by the type of score that is on.
        :rtype: pandas.DataFrame
        """
        dfs = []
    ...
    def get_csv_data(self, top = False, summary = False):
        """
        Get data by converting everything to a dataframe dataframe first.
        For now, one function pretty much does everything.

        :rtype: [dataframe.Dataframe],[str]
        ...
        def output_all_stats():
            ...
            for score_class in scores:
                ...
                for strategy in self.strategies:
                    ...
                    for tup in sorted(list(data.keys()), reverse=reverse):
                        triple = data[tup]
                        OUTFILE.write(
            ...
            for decoy in all_data:
                ...
                for score_class in scores:
                    ...
                    if decoy not in score_class.all_data[strategy]:
    ...
    def output_len_or_clus_alignment(self, alignment_type, features_type='antibody'):
        ...
        def _output_alignment(self, outdir, top_decoys, score, type_data, strategy=None, extra_name="top"):
            ...
            if self.native_path:
                ...
                if (cdr, "native") in all_type_data:
    ...
    def output_len_or_clus_recovery(self, alignment_type, features_type='antibody'):
        ...
        if self.individual_analysis.get():
            ...
            for strategy in self.strategies:
                outname = strategy
                decoys = list(scores[0].get_strategy_data(strategy).keys())
        ...
        if self.combined_analysis.get():
            ...
            _add_recovery_line(self, outname, list(scores[0].get_concatonated_map().keys()), data_class, OUTFILE)
            for score in scores:
                if score.name == "combined_str_score": continue
                top_decoys = score.get_ordered_decoy_list_all(top_n)
    ...
    def output_len_or_clus_enrichment(self, alignment_type, features_type='antibody'):
        ...
        if self.combined_analysis.get():
            outdir = self._setup_outdir_combined(["enrichment"])
            outname = "combined_all"
            _add_enrichments(self, outname, list(scores[0].get_concatonated_map().keys()), data_class)
            for score in scores:
                if score.name == "combined_str_score": continue
                top_decoys = score.get_ordered_decoy_list_all(top_n)
                outname = "combined_" + score.get_outname() + "_top_" + repr(top_n)
...
def calculate_recovery(native_data, all_decoy_data, cdr, decoy_list=None):
    ...
    if not decoy_list:
        decoy_list = list(raw_decoy_map.keys())
