        ...
        for db_path in self.db_paths:
            ...
            for row in cur.execute(query):
                ...
                if rr == None:
                    rr = 0
                #print repr(row)
                if pdb not in self.recovery_data[exp_name]:
...
class PlotData:
    ...
    def get_xy_of_exp(self, exp, rec = True, skip_H3 = True):
        x = []
        y = []
        for pdb in list(self.native_data.data.keys()):
            for cdr in list(self.native_data.data[pdb].keys()):
    ...
    def plot_data(self, outname, rec = True):
        for exp in list(self.rec_data.recovery_data.keys()):
