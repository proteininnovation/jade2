...
class DecoyData(object):
    ...
    def get_pandas_dataframe(self, top_n = None, drop_dir_prfix = False):
        """
        Gets all data as a dataframe dataframe.  Uses the set name as the score.
    ...
    def _add_data(self, strategy, decoy_data_map):
        ...
        """
        if strategy not in self.all_data:
    ...
    def get_top_strategy_data(self, strategy, top_n, by_score_tuple = False):
        ...
        top_points = sorted(list(data.keys()), reverse = reverse)[0:top_n]
    ...
    def get_ordered_decoy_list(self, strategy, top_n = None):
        ...
        if top_n:
            ...
            data = self.get_strategy_data(strategy, True)
        #print repr(data)
        points = sorted(list(data.keys()), reverse = reverse)
        #print points
        for tup in points:
    ...
    def get_ordered_decoy_list_all(self, top_n = None):
        ...
        decoys = []
        points = sorted(list(all_data.keys()), reverse = reverse)[0:top_n]
        for tup in points:
