...
def get_default_predictor(g: List, n_tasks = 1) -> pl.LightningModule:
    """
    Create the predictor using the graph list.
    :param g: List of DGL Graphs
    :return:
    """
    if g[0].ndata['data'].shape[1] > 64:
        model = MPNNPredictor(node_in_feats=g[0].ndata['data'].shape[1],
                              edge_in_feats=g[0].edata['data'].shape[1],
                              node_out_feats=64,
...
class MPNNGNN(pl.LightningModule):
    """MPNN.
...
class MPNNPredictor(pl.LightningModule):
